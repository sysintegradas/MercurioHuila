...
abstract class Tag {
	...
	public static function numericField($params){
		...
		$code = "<input id='".$params[0]."' value='$value' ";
		foreach($params as $key => $value){
			if(!is_integer($key)){
				$code.="$key='$value' ";
			}
		...
		if(is_array($params)){
			...
			if(is_object($params[1])){
				...
				foreach($params[1] as $o){
					if($callback==false){
                        $valor = "";
                        for($incre=1;$incre<count($using);$incre++){
                            if($using[0]==$using[$incre])$valor.=trim($o->readAttribute($using[$incre]))." - ";
                            if($using[0]!=$using[$incre])$valor.=trim($o->readAttribute($using[$incre]))." ";
                        }
                        if($value==$o->readAttribute($using[0])){

                            $code.="\t<option selected='selected' value='".trim($o->readAttribute($using[0]))."'>".trim($valor)."</option>\r\n";
                        } else {
                            $code.="\t<option value='".trim($o->readAttribute($using[0]))."'>".trim($valor)."</option>\r\n";
                        }
                    } else {
