...
var Validator = Class.create({
	...
	addField: function(name,type){
		...
		this._names.push(name);
          if(validate.isNull==false){
            if($(name).placeholder=="") $(name).placeholder = "Obligatorio";
          }
	},
	...
	valide: function(){//Realiza las validaciones.
		...
		for(var i=0;i<this._fields.length;i++){
			...
			if(this._fields[i].type != "select" && this._fields[i].type != "number" && this._fields[i].type != "decimal" && this._fields[i].type != "file" ){
			...
			if(this._fields[i].type.match(/^email$/) && !campo.value.match(/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/i) && !campo.value.blank())
